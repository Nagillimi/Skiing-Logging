...
def assignDownhillTileTracks(
    ...
    # get the start&stop timestamps for every run from the ground truth
    ...
    ranges = [[tile_start_idxs[i], tile_stop_idxs[i]] for i in range(len(tile_start_idxs))]

    tile_sync.file_train = createJumpDataFile(f'logs/tile-{truth[0].date}-jumps-{JUMP_THRESHOLD_MG}mG.csv')

    # split by these timestamps into [Tile]
    tile_sync.assignTracks(ranges)

    tile_sync.file_train.close()
